...
object ObjectOrientation extends App {
  ...
  // aDog.name is invalid
  // to promote argument to field, add val eg: class Dog(val name: String)

  // subtype polymorphism
  val aDeclaredAnimal: Animal = new Dog("Hachi")
  aDeclaredAnimal.eat() // the most defived method will be called at runtime eg Dog in this case

  // abstract class
  abstract class WalkingAnimal {
    val hasLegs = true // by default public, can restrict by adding protected or private
    def walk(): Unit // no implementation required becase derived class can define method
  }
  ...
  /*
    Benefits:
    1) works miracles in multithreaded/destributed env
